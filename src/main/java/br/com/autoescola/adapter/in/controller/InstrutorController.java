...
public class InstrutorController {
    ...
    @PostMapping
    public ResponseEntity<InstrutorDetailedResponseDTO> cadastrarInstrutor(
            @RequestBody @Valid InstrutorCreateDTO dados,
            UriComponentsBuilder uriBuilder) {
        InstrutorDetailedResponseDTO dto = service.cadastrarInstrutor(dados);
        URI uri = uriBuilder
                .path("/instrutores/{id}")
    ...
    @GetMapping("/{id}")
    public ResponseEntity<InstrutorDetailedResponseDTO> detalharInstrutor(@PathVariable Long id) {
        return ResponseEntity.ok(service.detalharInstrutor(id));
    }

    @PutMapping("/{id}")
    public ResponseEntity<InstrutorDetailedResponseDTO> atualizarInstrutor(
